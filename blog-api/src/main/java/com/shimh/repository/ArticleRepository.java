...
public interface ArticleRepository extends JpaRepository<Article, Integer>, ArticleWrapper {
    ...
    @Query(value = "select * from me_article where id = query_id order by view_counts desc limit :limit", nativeQuery = true)
    List<Article> findOrderByViewsAndLimit(@Param("limit") int limit);

    @Query(value = "select * from me_article where id = query_id order by create_date desc limit :limit", nativeQuery = true)
