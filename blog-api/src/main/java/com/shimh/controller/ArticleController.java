...
public class ArticleController {
    ...
    @FastJsonView(
            ...
            include = {@FastJsonFilter(clazz = User.class, props = {"id", "nickname", "avatar", "queryId"})})
    ...
    @RequiresAuthentication
    public Result acceptArticle(@PathVariable("id") Integer id) {
        ...
        Integer articleId = articleService.acceptArticle(id);
        if (articleId < 0) {
            r.setResultCode(ResultCode.DATA_IS_WRONG);
            System.out.println("articleId小于0");
            return r;
        }
        r.setResultCode(ResultCode.SUCCESS);
        r.simple().put("articleId", articleId);
