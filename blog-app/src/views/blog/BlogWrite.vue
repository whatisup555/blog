    ...
    <el-container>
      <base-header :simple=true>
        <el-col :span="4" :offset="2">
          <div class="me-write-info">开始写</div>
        </el-col>
      ...
      <el-dialog title="摘要 分类 标签"
        ...
        <el-form :model="articleForm" ref="articleForm" :rules="rules">
          ...
          <el-form-item label="分类" prop="category">
            <el-select v-model="articleForm.category" value-key="id" placeholder="请选择分类">
              <el-option v-for="c in categorys" :key="c.id" :label="c.categoryname" :value="c"></el-option>
            </el-select>
          </el-form-item>

          <el-form-item label="去处类型" prop="tags">
          ...
          <el-cascader
            ...
            size = "small"
            :options="pcaTextArr"
            v-model="articleForm.city">
          </el-cascader>
          </el-form-item>

          <!-- 向articleForm.time中输入请求持续天数 -->
          <el-form-item label="持续天数">
            <el-input-number v-model="articleForm.time" :min="1" :max="30" label="天"></el-input-number>
          </el-form-item>
        </el-form>
    ...
    methods: {
        ...
        this.$refs[articleForm].validate((valid) => {
          if (valid) {
            ...
            let article = {
              ...
              time: this.articleForm.time,
              city: this.articleForm.city,
              queryId: this.articleForm.id,
      ...
      getCategorysAndTags() {
        ...
        getAllCategorys().then(data => {
          that.categorys = data.data.slice(0,1)
        }).catch(error => {
          if (error !== 'error') {
