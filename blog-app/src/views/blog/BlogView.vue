    ...
    <el-container class="me-view-container">
      ...
      <el-main>
        ...
        <div class="me-view-card">
          ...
          <template v-if="this.article.id==this.article.queryId">
            <div class="me-view-tag" v-if="this.article.author.id==this.$store.state.id">
              
            </div>
            <div class="me-view-tag" v-else>
              <el-button @click="reponseWrite(article.id)" size="mini" type="primary" round plain>响应</el-button>
            </div>
          </template>
          <template v-else>
            <div class="me-view-tag" v-if="this.article.author.id==this.$store.state.id"></div>
            <div class="me-view-tag" v-else>
              <el-button @click="acceptArticle(article)" size="mini" type="primary" round plain>接受</el-button>
            </div>
          </template>
...
<script>
  ...
  import {viewArticle,accept} from '@/api/article'
  ...
  export default {
    ...
    computed: {
      ...
      title() {
        return `${this.article.title} - 文章 - 好去处`
      }
    },
    methods: {
      acceptArticle(article){
        accept(article).then(data => {
          this.$message({type: 'success', message: '接受成功', showClose: true})
        }).catch(error => {
          if (error !== 'error') {
            this.$message({type: 'error', message: '接受失败', showClose: true})
          }
        })
        this.$router.push({path: `/`})
      },
      reponseWrite(id) {
        this.$store.articleId = id
        this.$router.push({path: `/responseWrite/`})
      },
